...
let TMP_KEY_IGNORE:any = 'node-ignore'
...
const makeRegex = (pattern, ignoreCase) => {
    ...
    if (!source) {
        source = REPLACERS.reduce(
            (prev, current) => prev.replace(current[0], (current[1] as any).bind(pattern)),
            pattern
        )
...
export class Ignore {
    ...
    _testOne(path, checkUnignored) {
        ...
        this._rules.forEach(rule => {
            ...
            const matched = rule.regex.test((rule.negative ? '!' : '') +path)
    ...
    // @returns {TestResult}
    _test(originalPath, cache, checkUnignored, slices = false) {
    ...
    _t(path, cache, checkUnignored, slices) {
        ...
        let mayBeSubUnignore = false;
        for (const r of this._rules) {
            if(r.negative && r.regex.test('!'+path)){
                mayBeSubUnignore = true;
            }
        }
        if(mayBeSubUnignore){
